...
if __name__=='__main__':
    ...
    for sentence in doc.sents:
        ...
        for token in sentence:
            ...
            if (token.dep_ == "nsubj" or token.dep_ == "nsubj:pass") and (case not in wrong_padezh or case == 'loct') and case != None:
            ...
            if token.dep_ == "conj" and token.pos_ in ["VERB", "ADJ"] and case not in wrong_padezh and token.text[-2:] != "сь"\
            ...
            if token.dep_ == "ccomp" and token.pos_ == "VERB" and case not in wrong_padezh and token.text[-2:] != "сь":
                predicate.append(token.text)
            if token.dep_ == "parataxis" and token.pos_ == "VERB" and case not in wrong_padezh and token.text[-2:] != "сь":
                predicate.append(token.text)
